...
class ECABottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=1, bias=False)  # no biases
        self.bn1 = nn.BatchNorm2d(planes)
        ...
        self.squeeze1 = nn.AdaptiveMaxPool2d(1)  # AdaptiveMaxPooling
        self.eca = eca_layer(planes * 4)
        self.downsample = downsample
        self.stride = stride

        self.a = torch.rand(1)
        self.e = torch.tensor([0.78])     # Factor of probability
...
class IPCPA_ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=6): # Number of categories, modified as needed
        self.inplanes = 64
        super(IPCPA_ResNet, self).__init__()
...
def IPCPA_ResNet_18():
    return IPCPA_ResNet(ECABottleneck, [2, 2, 2, 2])
def IPCPA_ResNet_34():
    return IPCPA_ResNet(ECABottleneck, [3, 4, 6, 3])
def IPCPA_ResNet_50():
    return IPCPA_ResNet(ECABottleneck, [3, 4, 6, 3])
def IPCPA_ResNet_101():
    return IPCPA_ResNet(ECABottleneck, [3, 4, 23, 3])
